...
def _heuristic_findings(clauses: List[Dict], contract_id: str, run_id: str) -> List[Dict]:
    """Backend-safe fallback when Gemini is not configured or unavailable."""
    findings = []
    patterns = [
        ("violation", "DPDP Section 6", "DPDP", "processing personal data without valid consent", ("without consent", "no consent")),
        ("high", "DPDP Section 8(4)", "DPDP", "indefinite data retention may violate storage limitation", ("indefinite", "forever", "perpetual")),
        ("high", "RBI DPSS Circular 2018", "RBI", "payment data may require India-local storage", ("payment data", "transaction data")),
        ("high", "GDPR Article 44", "GDPR", "cross-border transfer needs safeguards", ("outside india", "third country", "united states")),
        ("medium", "GDPR Article 28", "GDPR", "sub-processor rights require notice and controls", ("subprocessor", "sub-processor", "subcontract")),
        ("medium", "DPDP Section 8", "DPDP", "audit and accountability rights should be explicit", ("no audit", "without audit")),
    ]
    for clause in clauses[:30]:
        text = clause.get("text", "")
        lower = text.lower()
        match = next((p for p in patterns if any(term in lower for term in p[4])), None)
        if match:
            severity, ref, body, issue, _ = match
            confidence = 0.86 if severity in {"high", "violation"} else 0.78
        elif "personal data" in lower or "customer data" in lower:
            severity, ref, body, issue, confidence = (
                "medium",
                "DPDP Section 6",
                "DPDP",
                "data processing clause should state consent, purpose, and deletion obligations",
                0.74,
            )
        else:
            severity, ref, body, issue, confidence = (
                ...
                "NONE",
                "NONE",
                "No major compliance issue detected by offline checks",
                0.82,
            )
        findings.append({
            "contract_id": contract_id,
            "run_id": run_id,
            "clause_index": int(clause.get("index", 0)),
            "clause_text": text,
            "severity": severity,
            "confidence": confidence,
            "regulation_ref": ref,
            "regulation_body": body,
            "issue": issue,
            "needs_retry": confidence < CONFIDENCE_THRESHOLD,
        })
    return findings
...
async def classify_clauses(
    ...
    try:
        ...
        print(f"[CLASSIFIER] Gemini unavailable, using offline classifier: {e}")
        return _heuristic_findings(clauses, contract_id, run_id)
    if not findings:
        return _heuristic_findings(clauses, contract_id, run_id)
